...
async function getGoogleAuthToken(): Promise<string> {
  ...
  // Remove quotes and escaped newlines
  privateKey = privateKey.replace(/\\n/g, "\n").replace(/["']/g, "").trim();
  
  // If the key is all on one line or missing headers, add them
  if (!privateKey.includes("BEGIN PRIVATE KEY")) {
    // Remove any existing line breaks from the base64 content
    const keyContent = privateKey.replace(/[\r\n]/g, '');
    privateKey = `-----BEGIN PRIVATE KEY-----\n${keyContent}\n-----END PRIVATE KEY-----`;
  }
  
  // Ensure proper formatting: header, 64-char lines for base64, footer
  ...
  try {
    ...
    throw new Error(`Invalid private key format: ${errorMessage}. Please ensure GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY is properly formatted.`);
  }
