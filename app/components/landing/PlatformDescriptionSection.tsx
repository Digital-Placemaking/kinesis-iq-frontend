        ...
        >
          {featureCards.map((card, i) => {
            ...
            return (
              <motion.div
                key={card.title}
                className={`group relative p-8 rounded-xl border-2 ${card.borderColor} bg-zinc-900/50 backdrop-blur-sm hover:bg-zinc-900/80 hover:border-opacity-100 cursor-pointer overflow-hidden min-h-[280px]`}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-100px" }}
                transition={{
