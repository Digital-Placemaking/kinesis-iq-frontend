    ...
    <Card className="border-zinc-800 bg-zinc-900/50 backdrop-blur-sm h-full flex flex-col">
      <CardHeader className="pb-2 lg:pb-4 flex-shrink-0 px-2.5 lg:px-6">
        <CardTitle className="text-lg lg:text-xl font-bold text-white flex items-center gap-2 lg:gap-3">
          <MapPin className="h-5 w-5 lg:h-6 lg:w-6 text-purple-400 shrink-0" />
      ...
      <CardContent className="pt-2 flex flex-col flex-1 min-h-0 px-2.5 lg:px-6">
        {isLoading ? (
          ...
          <div className="flex flex-col flex-1 min-h-0 space-y-2 lg:space-y-2.5">
            ...
            <div className="space-y-1.5 lg:space-y-2 pt-2 lg:pt-2.5 border-t border-zinc-800 flex-shrink-0">
              {data.map((location) => (
                <div
                  key={location.name}
                  className="flex items-center justify-between py-1.5 lg:py-2 px-2 lg:px-3 rounded-lg bg-zinc-900/30 hover:bg-zinc-900/50 transition-colors"
                >
                  <div className="flex items-center gap-1.5 lg:gap-3 min-w-0 flex-1">
                    <MapPin className="h-3.5 w-3.5 lg:h-4 lg:w-4 text-purple-400 shrink-0" />
                    <span className="text-white font-semibold text-xs lg:text-sm truncate">
                      {location.name}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 lg:gap-4 shrink-0 ml-2 lg:ml-3">
                    <div className="flex items-center gap-1 lg:gap-1.5">
                      <Users className="h-3 w-3 lg:h-3.5 lg:w-3.5 text-zinc-400 shrink-0" />
                      <span className="text-zinc-300 font-medium text-xs lg:text-sm whitespace-nowrap">
                        {location.responses.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center gap-1 lg:gap-1.5">
                      <Star className="h-3 w-3 lg:h-3.5 lg:w-3.5 text-green-400 shrink-0" />
                      <span className="text-green-400 font-bold text-xs lg:text-sm whitespace-nowrap">
                        {location.sentiment}%
                      </span>
                    </div>
                  </div>
