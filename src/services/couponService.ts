...
import { isCouponScheduleValid } from '@/utils/couponSchedule';
...
export const claimCoupon = async (params: ClaimCouponParams): Promise<ClaimCouponResult> => {
  try {
    ...
    // First, fetch the coupon to check its schedule
    ...
    const scheduleValidation = isCouponScheduleValid({
      valid_days: couponData.valid_days,
      valid_time_start: couponData.valid_time_start,
      valid_time_end: couponData.valid_time_end
    });

    if (!scheduleValidation.valid) {
      return {
        success: false,
        message: scheduleValidation.message || 'Coupon is not currently available'
      };
    }
    ...
    // Fetch the actual coupon data with the real code (we already fetched it above for schedule validation)
    // No need to fetch again, use the couponData we already have
