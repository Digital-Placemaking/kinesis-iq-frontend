...
export const uploadCouponImage = async (
  ...
  try {
    ...
    const fileName = `${couponId}-${Date.now()}.${fileExt}`;
    const { data, error } = await supabase.storage
      .from('coupon-images')
      .upload(fileName, file, {
        cacheControl: '3600',
    ...
    if (updateError) {
      console.error('Error updating coupon:', updateError);
      return { success: false, message: 'Failed to update coupon with image' };
    }
    ...
    console.error('Error in uploadCouponImage:', error);
    return { success: false, message: 'Failed to upload image' };
  }
};
