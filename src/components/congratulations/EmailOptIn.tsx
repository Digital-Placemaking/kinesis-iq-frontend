...
export const EmailOptIn: React.FC<EmailOptInProps> = ({ onComplete, onSkip }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Record opt-in via secure edge function
      ...
      if (!result.success) {
        console.error("Error recording email opt-in:", result.error);
        ...
        <CardContent className="space-y-4 p-0">
          ...
          <div className="flex justify-center mt-4">
            <a 
              ...
              className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-10 px-4 py-2 bg-primary hover:bg-primary/90"
            >
              Return Home
            </a>
          </div>
      ...
      <CardContent className="space-y-4 p-0">
        <div className="text-center">
          <Label className="text-lg font-semibold">
            Your email unlocks instant rewards and insider perks.
          </Label>
