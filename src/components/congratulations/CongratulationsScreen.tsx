...
const CongratulationsScreen: React.FC<CongratulationsScreenProps> = ({ 
  ...
  // Check if coupon is already claimed (has a code and share_token)
    const isAlreadyClaimed = initialCoupon.code && initialCoupon.share_token;
    
    if (isAlreadyClaimed) {
      setClaimed(true);
      setCoupon(initialCoupon);
    } else if (!claimed && !isClaiming && initialCoupon.id !== 'no-coupon') {
      // Fallback: Auto-claim if not already claimed
      handleClaimCoupon();
    }
      ...
      <Card className="w-full">
        ...
        <CardFooter className="flex flex-col space-y-3">
          ...
          <Button 
            ...
            onClick={() => window.open('https://www.anthonyperruzza.com/', '_blank')}
          >
