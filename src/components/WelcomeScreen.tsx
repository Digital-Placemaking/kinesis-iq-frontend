...
import { collectEmail } from "@/services/emailCollectionService";
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({
  ...
  // Only auto-progress on actual OAuth returns (not existing sessions)
  useEffect(() => {
    const hashParams = new URLSearchParams(window.location.hash.substring(1));
    const searchParams = new URLSearchParams(window.location.search);
    const isOAuthReturn = hashParams.has('access_token') || searchParams.has('code');
    
    if (!isOAuthReturn) return;

    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        onStartSurvey(session.user.email);
      }
    });
  ...
  const handleStartWithEmail = async () => {
    ...
    try {
      ...
      // Record email via secure edge function
      ...
      if (!result.success) {
        ...
        toast.error(result.error || "There was an issue saving your email. Please try again.");
        setIsSubmitting(false);
        return;
      }

      // Track referral if code exists
      const storedReferralCode = referralCode || localStorage.getItem('referral_code');
      if (storedReferralCode) {
        const { error: referralError } = await supabase.rpc('track_referral_conversion', {
          p_referral_code: storedReferralCode,
          p_referred_email: sanitizedEmail,
          p_referred_device_id: deviceId
        });
        
        if (!referralError) {
          localStorage.removeItem('referral_code'); // Clear after tracking
        }
      }
  ...
  return <div className="text-center max-w-lg animate-fade-in">
      ...
      <div className="mt-10 mb-6">
        <img src="/lovable-uploads/anthony-perruzza-logo.jpg" alt="Anthony Perruzza - City Councillor, Humber River - Black Creek" className="mx-auto h-36 w-auto" onError={e => {
      ...
      {/* Headline */}
      <h1 className="text-3xl font-bold mb-4 font-playfair">Here to connect and support you with any City-related matter and/or concerns.</h1>

      {/* Subtext */}
      <p className="text-gray-600 mb-6">Exclusive Offers - Access Below</p>

      {/* Sign-in Options */}
      <div className="mb-6 space-y-3">
        {!showEmailInput ? (
          <>
            {/* Apple and Google Sign-in Side by Side */}
            <div className="flex gap-2">
              ...
              <Button 
                ...
                size="lg" 
                className="flex-1 bg-black hover:bg-gray-900 text-white transition-all transform hover:scale-105 active:scale-95 shadow-md flex items-center justify-center gap-2 text-sm"
              >
                <Apple size={18} />
              ...
              <Button 
                ...
                disabled={isSubmitting}
                size="lg" 
                className="flex-1 bg-white hover:bg-gray-50 text-gray-900 border-2 border-gray-200 transition-all transform hover:scale-105 active:scale-95 shadow-md flex items-center justify-center gap-2 text-sm"
              >
