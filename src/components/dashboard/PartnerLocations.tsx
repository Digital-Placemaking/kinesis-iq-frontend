...
const PartnerLocations: React.FC<PartnerLocationsProps> = ({ selectedPartner, onPartnerSelect }) => {
  ...
  const [editingPartner, setEditingPartner] = useState<{ id: string; name: string; slug: string; description: string } | null>(null);
  ...
  const handleEditPartner = async () => {
    if (!editingPartner?.name.trim() || !editingPartner?.slug.trim()) {
      toast.error('Name and slug are required');
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          slug: editingPartner.slug.trim(),
          description: editingPartner.description.trim() || null,
        })
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setEditingPartner(prev => prev ? { ...prev, slug: e.target.value } : null)}
                placeholder="location-slug"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={handleEditPartner} disabled={isUpdating}>
